...
pagetable_t
proc_kpt_init(){
  // printf("uvmmap: create kernel page table\n");
  pagetable_t kernelpt = uvmcreate();
  if (kernelpt == 0) {
    printf("uvmmap: failed to create kernel page table\n");
    return 0;
  }
  ...
  // PLIC
  uvmmap(kernelpt, PLIC, PLIC, 0x4000000, PTE_R | PTE_W);
...
// print pagetable
void
_vm_print(pagetable_t pagetable, int level)
{
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if(pte & PTE_V){
      ...
      if((pte & (PTE_R|PTE_W|PTE_X)) == 0){
        // this PTE points to a lower-level page table.
        _vm_print((pagetable_t)child, level + 1);
      }
    }
  }
